...
OTHER_PROPS: list[str] = sorted(
    [p for p in ALL_PROPS if p not in CORE_PROPS_SET and p not in THREED_PROPS_SET]
)
...
PROPERTY_ALIASES.update(
    {camel.lower(): snake for camel, snake in CAMEL_TO_SNAKE.items()}
)  # lowercase -> snake_case
PROPERTY_ALIASES.update(
    {snake.replace("_", ""): snake for snake in ALL_PROPS}
)  # flatcase -> snake_case
