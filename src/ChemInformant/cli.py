...
from .cheminfo_api import get_properties, draw_compound, PROPERTY_ALIASES, _SPECIAL_PROPS
...
def main_draw():
    ...
    parser.add_argument(
        "identifier",
        help="A single chemical identifier (Name, CID, or SMILES string)."
    )
    args = parser.parse_args()
    
    try:
        print(f"Attempting to draw structure for '{args.identifier}'...", file=sys.stderr)
        # 调用绘图函数
        ...
        print(f"[ChemInformant] An unexpected error occurred: {type(e).__name__} - {e}", file=sys.stderr)
        sys.exit(1)