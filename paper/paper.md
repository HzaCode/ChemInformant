...
| **Optimized Batch Retrieval**<br/>(Efficiency for Multiple Entities/Properties) | Provides `get_multiple_compounds` for **efficient retrieval of comprehensive chemical profiles** for multiple compounds, thereby minimizing API calls. This is advantageous for screening large chemical datasets.        | The `get_properties` function allows batch retrieval of a specified, limited set of core chemical properties. Fetching diverse property types for numerous compounds may require iterative API calls or custom aggregation logic. |
