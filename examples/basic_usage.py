...
try:
    ...
    print("Please ensure you have installed the package by running 'pip install .' in the project root directory.")
    sys.exit(1)
...
def demonstrate_single_compound(compound_identifier):
    ...
    print_separator(f"Single Query: {compound_identifier} (Type: {type(compound_identifier).__name__})")
    ...
    try:
        ...
        if compound_cid is None:
             print(f"  - Could not definitively resolve CID for '{compound_identifier}' (maybe not found or ambiguous)")
        ...
        print(f"  - CAS: {compound_cas} (Query took: {end_time_cas - start_time_cas:.4f}s)")
        ...
        print(f"  - Weight: {compound_weight:.2f}" if compound_weight is not None else "N/A")
        ...
        compound_info: ci.CompoundData = ci.info(compound_identifier) # Calling ci.info()
        ...
        print(f"    Description snippet: {(compound_info.description[:60] + '...') if compound_info.description and len(compound_info.description) > 60 else compound_info.description}")
        print(f"    First Synonym: {compound_info.synonyms[0] if compound_info.synonyms else 'N/A'}")
    ...
    # Catch our defined specific errors
        ...
        print(f"  Error: Identifier '{e.identifier}' is ambiguous, maps to CIDs: {e.cids}")
        # Logic to handle ambiguity could be added here, e.g., selecting the first CID to requery
        if e.cids:
            first_cid = e.cids[0]
            print(f"    Attempting to requery info() with the first CID ({first_cid})...")
            try:
                specific_info = ci.info(first_cid) # Query with the specific CID
                print(f"    Requery successful: Formula={specific_info.molecular_formula}")
...
def demonstrate_batch():
    ...
    identifiers = ["Water", 2244, "Glucose", "NonExistentCompound", "Ethanol", "Benzene", "AmbiguousDrug"]

    print(f"Input identifier list: {identifiers}")
    start_time_batch = time.perf_counter()
    results = ci.get_multiple_compounds(identifiers) # Calling ci.get_multiple_compounds()
    ...
    for identifier, result in results.items():
        ...
        if isinstance(result, ci.CompoundData): # Using ci.CompoundData
            ...
            print(f"  - {id_repr:<25}: Success - CID={result.cid:<5} Formula={result.molecular_formula:<10} CAS={result.cas}")
        elif isinstance(result, ci.AmbiguousIdentifierError): # Using ci.AmbiguousIdentifierError
        ...
        elif isinstance(result, Exception): # Catch other retrieval or processing errors
             print(f"  - {id_repr:<25}: Failed - Unexpected error: {type(result).__name__}: {result}")
...
if __name__ == "__main__":
    ...
    demonstrate_single_compound("Glucose") # Check ambiguity handling (depends on current PubChem data)
